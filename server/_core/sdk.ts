...
class OAuthService {
  constructor(private client: ReturnType<typeof axios.create>) {
    if (!ENV.oAuthServerUrl) {
      console.log("[OAuth] Running in local mode without OAuth configuration");
    } else {
...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<User> {
    // Development mode: bypass OAuth if not configured
    if (!ENV.oAuthServerUrl || ENV.oAuthServerUrl.trim() === "") {
      console.log("[Auth] Development mode: OAuth not configured, using local admin user");
      
      // Check if local admin user exists
      let adminUser = await db.getUserByOpenId("local-admin");
      ...
      if (!adminUser) {
        ...
        adminUser = await db.getUserByOpenId("local-admin");
        console.log("[Auth] Created local admin user");
      } else {
